...
class Average_General_Poll_Filter_Table extends WP_List_Table {
    ...
    {
        $q['q1'] = $q['q2'] = $q['q3'] = $q['prom'] = $q['qty_poll'] = 0;
        $dat = array();
        foreach($this->bodegas_qn as $key => $bodega) {            
            $q['q1'] += $bodega['q1'];
            $q['q2'] += $bodega['q2'];
            $q['q3'] += $bodega['q3'];
            $q['qty_poll'] += $bodega['qty_orders'];
            $q['prom'] += (floatval($bodega['q1']) + floatval($bodega['q2']) + floatval($bodega['q3'])) / $this->base;
        }
        $qty_bodegas = count($this->bodegas_qn);
        $dat['q1'] = $this->format_number_prom($q['q1'] / $qty_bodegas);
        $dat['q2'] = $this->format_number_prom($q['q2'] / $qty_bodegas);
        $dat['q3'] = $this->format_number_prom($q['q3'] / $qty_bodegas);        
        $dat['q4'] = "N/A";
        $dat['qty_poll'] = $this->format_number_prom($q['qty_poll']);
        $dat['prom'] = $this->format_number_prom($q['prom'] / $qty_bodegas);
